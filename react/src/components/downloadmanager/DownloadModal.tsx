...
const DownloadModal = ({status, createdAt, isModalOpen, handleModalClose}: DownloadModalProps) => {
  return (
          <Dialog
            open={isModalOpen}
          ...
          <DialogContent sx={{ display: 'flex', justifyContent: 'center', flexDirection: 'column', textAlign: 'center'}}>
            ...
            <Button sx={{margin: '1em auto'}} href={`${ApiClient.exportEndpoint}/${createdAt}`} disabled={status !== 'complete'} color='primary'>Download</Button>
          </DialogContent>
          </Dialog>
  )
}
