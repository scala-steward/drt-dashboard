...
export function ViewDropInSession(props: Props) {
    ...
    <Dialog open={props.view} maxWidth="sm" onClose={handleViewClose}>
      ...
      <DialogContent sx={{
        ...
        <Grid container spacing={3} alignItems="center">
          ...
          <Grid item xs={9}>
            <Typography>{moment(props.startTime).format("HH:mm, Do MMM YYYY")}</Typography>
          </Grid>
          <Grid item xs={3}>
            <label>End Time:</label>
          </Grid>
          <Grid item xs={9}>
            <Typography>{moment(props.endTime).format("HH:mm, Do MMM YYYY")}</Typography>
          </Grid>
          <Grid item xs={3}>
