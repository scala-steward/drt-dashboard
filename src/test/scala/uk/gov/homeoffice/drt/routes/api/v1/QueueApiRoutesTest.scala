...
class QueueApiRoutesTest extends AnyWordSpec with Matchers with ScalatestRouteTest {
  ...
  "Given a request for the queue status, I should see a JSON response containing the queue status" in {
    ...
    val queueApiRoutes = QueueApiRoutes(
    ...
    Get("/v1/queues?start=" + start + "&end=" + end) ~>
      ...
      responseAs[String] shouldEqual QueueApiRoutes.JsonResponse(start, end, defaultPeriodMinutes, Seq(portContent, portContent)).toJson.compactPrint
    }
  }
