...
import uk.gov.homeoffice.drt.routes.api.v1_1.AuthApiV1_1Routes.Credentials

import scala.concurrent.{ExecutionContextExecutor, Future}

class AuthApiV1_1RoutesTest extends AnyWordSpec with Matchers with ScalatestRouteTest {
  ...
  "A post request with a valid username and password should return a token" in {
    ...
    val routes = AuthApiV1_1Routes((_, _) => Future.successful(token))
