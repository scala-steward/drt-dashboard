...
object FlightExportV1 {
  def flights(flightsForDatesAndTerminals: (PortCode, List[FeedSource], LocalDate, LocalDate, Seq[Terminal]) => Source[ApiFlightWithSplits, NotUsed])
             (implicit ec: ExecutionContext, mat: Materializer): Seq[PortCode] => (SDateLike, SDateLike) => Future[FlightJsonResponseV1] =
    portCodes => (start, end) => {
      ...
      Source(portCodes)
            ...
            flightsForDatesAndTerminals(portCode, sourceOrder, dates.min, dates.max, Seq(terminal))
              ...
              .map {
                _
                  ...
                  .map(f => FlightJsonV1(portCode, f.apiFlight))
              }
          }
        ...
        .runWith(Sink.fold(Seq.empty[FlightJsonV1])(_ ++ _))
        .map(FlightJsonResponseV1(start, end, _))
    }
}
