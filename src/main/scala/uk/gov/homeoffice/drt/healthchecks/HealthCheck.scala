...
case class QueueApiV1HealthCheck(now: () => SDateLike, portCodes: Iterable[PortCode]) extends JsonHealthCheck[QueueJsonResponseV1_1] with QueueApiV1JsonFormats {
...
case class FlightApiV_11HealthCheck(now: () => SDateLike, portCodes: Iterable[PortCode]) extends JsonHealthCheck[FlightJsonResponseV1_1] with FlightApiV1_1JsonFormats {
  override val priority: IncidentPriority = Priority1
  override val name: String = "Flight API v1"
  override def description: String = s"Flight API v1 is reachable and responding with valid json"
